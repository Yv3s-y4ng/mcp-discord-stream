export const toolList = [
  ...
  },
  {
    ...
    description: "Streams messages from a Discord channel with time range filtering (default: last 10 days). Returns all message events including progress updates.",
    inputSchema: {
      type: "object",
      properties: {
        channelId: { type: "string", description: "The ID of the channel to read messages from" },
        afterDate: { type: "string", description: "ISO 8601 date string - only fetch messages after this date" },
        beforeDate: { type: "string", description: "ISO 8601 date string - only fetch messages before this date" }
      },
      required: ["channelId"]
    }
  },
  {
    ...
    description: "Streams messages from multiple Discord channels at once. Processes each channel sequentially with time range filtering.",
    inputSchema: {
      type: "object",
      properties: {
        channelIds: {
          type: "array",
          items: { type: "string" },
          description: "Array of channel IDs to read messages from"
        },
        afterDate: { type: "string", description: "ISO 8601 date string - only fetch messages after this date" },
        beforeDate: { type: "string", description: "ISO 8601 date string - only fetch messages before this date" }
      },
  ...
  },
  {
    ...
    description: "Lists all channels in a Discord server with optional filtering by channel type (text, voice, forum, etc.)",
    inputSchema: {
      type: "object",
      properties: {
        guildId: { type: "string", description: "The ID of the Discord server (guild)" },
        channelTypes: {
          type: "array",
          items: { type: "number" },
          description: "Optional array of Discord ChannelType numbers to filter by (0=text, 2=voice, 4=category, 15=forum, etc.)"
        }
      },
      required: ["guildId"]
    }
  },
  {
    ...
    description: "Streams all messages from a single forum post/thread with time range filtering (default: last 10 days)",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Archives an entire forum channel by streaming all threads and their messages. Includes active and archived threads with time range filtering.",
    inputSchema: {
      type: "object",
      properties: {
        forumChannelId: { type: "string", description: "The ID of the forum channel to archive" },
        afterDate: { type: "string", description: "ISO 8601 date string - only fetch messages after this date" },
        beforeDate: { type: "string", description: "ISO 8601 date string - only fetch messages before this date" },
        includeArchived: { type: "boolean", description: "Whether to include archived threads (default: true)", default: true }
      },
  ...
  },
  {
    ...
    description: "Archives multiple forum channels at once. Processes each forum sequentially, collecting all threads and messages.",
    inputSchema: {
      type: "object",
      properties: {
        forumChannelIds: {
          type: "array",
          items: { type: "string" },
          description: "Array of forum channel IDs to archive"
        },
        afterDate: { type: "string", description: "ISO 8601 date string - only fetch messages after this date" },
        beforeDate: { type: "string", description: "ISO 8601 date string - only fetch messages before this date" },
        includeArchived: { type: "boolean", description: "Whether to include archived threads (default: true)", default: true }
